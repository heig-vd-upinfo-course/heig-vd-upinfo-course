...
basique et n'a aucune intelligence propre (il n'est pas capable de réfléchir ou
...
3. Dans la fenêtre qui s'ouvre, recherchez la section "Spécifications de
   l'appareil" pour voir le fabricant, le modèle et l'architecture de votre
   processeur.
...
3. Dans la fenêtre qui s'ouvre, vous verrez les informations sur votre
   processeur (nommé "Puce"), y compris le fabricant, le modèle et
   l'architecture. Si votre Mac utilise un processeur Apple, il s'agit d'une
   architecture ARM. Si c'est un processeur Intel, il s'agit d'une architecture
   x86/x64.
...
3. Recherchez les lignes "Architecture" et "Modèle du processeur" pour voir le
   fabricant, le modèle et l'architecture de votre processeur.
